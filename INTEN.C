...
/*   initgraph(&gdriver, &gmode,""); */
   ...
   for (j=1; j<info.biHeight-1; j++)
   {
	  ...
	  {
		 inten = color_table[cur_bitmap[i] & 0xF0].rgbBlue +
				 color_table[cur_bitmap[i] & 0xF0].rgbRed +
				 color_table[cur_bitmap[i] & 0xF0].rgbGreen;
		 start = insert_node(start, abs(inten));
		 total = total + 1;
		 inten = color_table[cur_bitmap[i] & 0x0F].rgbBlue +
   ...
   while (temp != NULL)
   {
	   ...
	   line(100+i,450, 100+i, 450 - (temp->freq/100));
	   temp = temp->next ;
	   i = i + 7;
   }
   fflush(stdin) ;
